...
class ExampleChatRoute extends StatefulWidget {
  const ExampleChatRoute({super.key});

  @override
  State<ExampleChatRoute> createState() => _ExampleChatRouteState();
}

class _ExampleChatRouteState extends State<ExampleChatRoute> {
  ...
  static const String _description =
      "La seguente descrizione ed i seguenti messaggi sono necessari per guidare il comportamento del modello verso l'approccio che desideriamo.";
  ...
  void initState() {
    ...
    for (int i = 0; i < sentences.length - 1; i++) {
      ...
      if (i % 2 == 0) {
        messages
            .add(Message(sentences[i].replaceFirst("Io: ", ""), sender: "Io"));
      } else {
        messages.add(Message(sentences[i].replaceFirst("Jane: ", "")));
      }
    }
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: const ChatAppBar(title: "Jane", imagePath: "images/jane.png"),
